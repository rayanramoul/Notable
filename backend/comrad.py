...
class Comrad():
    ...
    def crossover(self, comrad):
        ...
        size = random.choice(pick)
        newrep = []
        for i in range(size):
            pick = []
            try:
                pick.append(self.representation[i])
                pick.append(comrad.representation[i])
                cho = random.choice(pick)
            except:
    ...
    def learn(self, learnX, learnY, validationX, validationY):
        ...
        for t in range(self.epochs):
            print("Epoch ("+str(t+1)+"/"+str(self.epochs)+")")
    ...
    def save(self):
        torch.save(self.model, "./backend/models/trained_model")